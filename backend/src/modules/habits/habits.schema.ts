...
export const createHabitSchema = z.object({
  ...
  xpReward: z.number({invalid_type_error: 'XP reward must be a number'}).int('XP reward must be a whole number').min(10,  'XP reward must be at least 10').max(50, 'XP reward cannot exceed 50').default(20),
  attribute: z.enum(['strength', 'agility', 'intellect']).default('strength'),
});
...
export const updateHabitSchema = z.object({
  ...
  xpReward: z.number().int('XP reward must be a whole number').min(10,  'XP reward must be at least 10').max(50, 'XP reward cannot exceed 50').optional(),
  attribute: z.enum(['strength', 'agility', 'intellect']).optional(),
