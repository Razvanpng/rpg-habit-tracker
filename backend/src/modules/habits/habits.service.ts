...
function toHabitDto(habit: { id: string; userId: string; name: string; description: string | null; xpReward: number; isCompletedToday: boolean; createdAt: Date; updatedAt: Date; }): HabitDto {
  return { id: habit.id, userId: habit.userId, name: habit.name, description: habit.description, xpReward: habit.xpReward, isCompletedToday: habit.isCompletedToday, createdAt: habit.createdAt, updatedAt: habit.updatedAt };
}
...
export async function completeHabit(habitId: string, userId: string): Promise<CompleteHabitResult> {
  ...
  return await prisma.$transaction(async (tx) => {
    ...
    if (!habit) throw NotFoundError('Habit');
    if (habit.isCompletedToday) throw BadRequestError('This habit has already been completed today');
