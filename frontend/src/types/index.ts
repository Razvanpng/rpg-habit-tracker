...
export interface User { id: string; email: string; level: number; currentXp: number; xpToNextLevel: number; strength: number; agility: number; intellect: number; selectedClass: string; createdAt: string; }
export interface AuthPayload { user: User; authenticated: boolean; }
export interface Habit { id: string; userId: string; name: string; description: string | null; xpReward: number; attribute: string; isCompletedToday: boolean; createdAt: string; updatedAt: string; }
...
export interface HabitFormValues { name: string; description?: string; xpReward: number; attribute: string; }
